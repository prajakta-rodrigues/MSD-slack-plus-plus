...
/**
 * This class is similar to the java.io.PrintWriter class, but this class's methods work with our
 * non-blocking Socket classes. This class could easily be made to wait for network output (e.g., be
 * made &quot;non-blocking&quot; in technical parlance), but I have not worried about it yet.
 *
...
public class NetworkConnection implements Iterable<Message> {
  ...
  /**
   * Creates a new instance of this class. Since, by definition, this class sends output over the
   * network, we need to supply the non-blocking Socket instance to which we will write.
   *
   ...
   * over the SocketNB instance with which the PrintNetNB was instantiated. This returns whether our
   * attempt to send the message was successful.
   *
