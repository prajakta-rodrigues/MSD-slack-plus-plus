...
/**
 * This class manages the connection between an the IM client and the IM server. Instances of this
 * class can be relied upon to manage all the details of this connection and sends alerts when
 * appropriate. Instances of this class must be constructed and connected before it can be used to
 * transmit messages.
 *
...
public class IMConnection {
  ...
  /**
   * Real Connection instance which this class wraps and makes presentable to the user
   */
  ...
  /**
   * List of instances that have registered as a listener for received message events.
   */
  ...
  /**
   * Creates an instance that will manage a connection with an IM server, but does not begin the
   * process of making a connection to the IM server.
   *
  ...
  /**
   * Add the given listener to be notified whenever 1 or more Messages are received from IM server
   * via this connection.
   *
   * @param listener Instance which will begin to receive notifications of any messages received by
   * this IMConnection.
   * @throws InvalidListenerException Exception thrown when this is called with a value of null for
   * {@code listener}
   */
   ...
   * automatically be logged in to the server, even if there is already someone with that
   * username.<br/> Precondition: connectionActive() == false
   *
   * @return True if the connection was successfully made; false otherwise.
   * @throws IllegalNameException Exception thrown if we try to connect with an illegal username.
  ...
  /**
   * Returns whether the instance is managing an active, logged-in connection between the client and
   * an IM server.
   *
   * @return True if the client is logged in to the server using this connection; false otherwise.
   */
  ...
  /**
   * Break this connection with the IM server. Once this method is called, this instance will need
   * to be logged back in to the IM server to be usable.<br/> Precondition: connectionActive() ==
   * true
   */
  ...
  /**
   * Gets an object which can be used to read what the user types in on the keyboard without
   * waiting. The object returned by this method should be used rather than {@link Scanner} since
   * {@code Scanner} will cause a program to halt if there is no input.
   *
   * @return Instance of {@link KeyboardScanner} that can be used to read keyboard input for this
  ...
  /**
   * Gets an object which can be used to get the message sent by the server over this connection.
   * This is the only object that can be used to retrieve all these messages.
   *
   * @return Instance of {@link MessageScanner} that can be used to read message sent over this
   * connection for this user.
   */
  public MessageScanner getMessageScanner() {
    if (messageScanner == null) {
      throw new IllegalOperationException(
          "Cannot get a MessageScanner if you have not connected to the server!");
    }
   ...
   *
   * @return Current value of the user name and/or the username with which we logged in to this IM
   * server.
   */
  ...
  /**
   * Unless this is a &quot;special&quot; server message, this sends the given message to all of the
   * users logged in to the IM server. <br/> Precondition: connectionActive() == true
   *
  ...
  public void sendMessage(String message) {
    if (!connectionActive()) {
      throw new IllegalOperationException(
          "Cannot send a message if you are not connected to a server!\n");
    }
    Message sentMessage;
    if (message.charAt(0) == '/') {
  ...
  protected boolean login() {
    ...
    try {
      ...
      // Report the error
      System.err
          .println("ERROR:  Could not make a connection to: " + hostName + " at port " + portNum);
      System.err.println(
  ...
  void fireSendMessages(List<Message> mess) {
  ...
  @SuppressWarnings("unchecked")
  void fireStatusChange(String userName) {
